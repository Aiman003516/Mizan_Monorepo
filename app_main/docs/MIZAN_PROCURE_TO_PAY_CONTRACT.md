...
This contract defines the first procure-to-pay slice for Mizan. It introduces purchase requisitions, purchase orders, receipts and returns, and a server-enforced three-way matching evidence contract that compares purchase-order commitments, received quantities, and vendor-bill quantities. The `assert_purchase_bill_match(uuid)` RPC now provides a server-side eligibility gate that future bill approval/posting commands must call.
...
The implementation exposes line-level evidence and blocking reasons, and `assert_purchase_bill_match(uuid)` rejects a bill when any line is blocked. It does not automatically post vendor bills, mutate inventory, or approve exceptions. Existing or future bill approval/posting commands must call the gate server-side and require either a fully matched result or a separate governed exception approval. Exception approval is a separate governed action and must be implemented after the evidence contract is verified.
