...
class Bootstrap {
  ...
  static Future<List<Override>> init() async {
    ...
    if (sharedPrefs.getString(PreferencesRepository.keyLocale) == null) {
      try {
        final String systemLocale = WidgetsBinding
            .instance
            .platformDispatcher
            .locale
            .toLanguageTag();
        ...
        } else if (systemLocale.startsWith('en-GB') ||
            systemLocale.startsWith('en_GB') ||
            systemLocale.startsWith('en_UK')) {
          detectedLang = 'en';
        ...
        await sharedPrefs.setString(
          PreferencesRepository.keyLocale,
          detectedLang,
        );
        await sharedPrefs.setString(
          PreferencesRepository.keyDefaultCurrency,
          detectedCurrency,
        );

        debugPrint(
          '🚀 Context Engine: Detected $systemLocale. Configured App -> Lang: $detectedLang, Currency: $detectedCurrency',
        );
      } catch (e) {
