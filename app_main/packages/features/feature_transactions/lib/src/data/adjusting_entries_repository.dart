...
// Provider
final adjustingEntriesRepositoryProvider = Provider<AdjustingEntriesRepository>(
  (ref) {
    ...
    return _db.transaction(() async {
      ...
      for (final entry in entries) {
        await _db
            ...
            .insert(entry.copyWith(transactionId: Value(newTransactionId)));
      }

      // D. Update the Task Status
      await (_db.update(
        _db.adjustingEntryTasks,
      )..where((t) => t.id.equals(task.id))).write(
        AdjustingEntryTasksCompanion(
