...
final orderForTransactionProvider = FutureProvider.autoDispose
    .family<Order?, String>((ref, transactionId) {
      final repo = ref.watch(transactionsRepositoryProvider);
      return repo.getOrderForTransaction(transactionId);
    });

final orderItemsStreamProvider = StreamProvider.autoDispose
    .family<List<OrderItem>, String>((ref, orderId) {
      final repo = ref.watch(transactionsRepositoryProvider);
...
class TransactionsRepository {
  ...
  Future<Order?> getOrderForTransaction(String transactionId) {
    return (_db.select(
      _db.orders,
    )..where((o) => o.transactionId.equals(transactionId))).getSingleOrNull();
  }
  ...
  Stream<List<Transaction>> watchAllTransactions() {
    return (_db.select(
      _db.transactions,
    )..orderBy([(t) => d.OrderingTerm.desc(t.transactionDate)])).watch();
  }
  ...
  Stream<List<TransactionEntry>> watchEntriesForTransaction(
    ...
    )..where((te) => te.transactionId.equals(transactionId))).watch();
  }

  // --- 🏭 THE COSTING ENGINE (Phase 3.2) ---

  Future<int> _calculateCOGSAndConsumeLayers(
    String productId,
    double quantitySold,
  ) async {
    // 1. Fetch User Preference
    final method = _prefsRepo
        .getInventoryCostingMethod(); // 'fifo', 'lifo', or 'weighted_average'

    // 2. Determine Sort Order
    // LIFO = Newest First (Desc)
    // FIFO = Oldest First (Asc) - Default
    var sortOrder = d.OrderingTerm.asc(_db.inventoryCostLayers.purchaseDate);
    if (method == 'lifo') {
      sortOrder = d.OrderingTerm.desc(_db.inventoryCostLayers.purchaseDate);
    }

    // 3. Query with Dynamic Sort
    final layers =
    ...
    // 4. Handle Weighted Average Special Case
    int waCostPerUnit = 0;
    ...
    for (var layer in layers) {
      ...
      int unitCostUsed = (method == 'weighted_average')
          ? waCostPerUnit
          : layer.costPerUnit;
      ...
      await (_db.update(
        ...
        InventoryCostLayersCompanion(
          quantityRemaining: d.Value(
            layer.quantityRemaining - qtyFromThisLayer,
          ),
        ),
      );
    ...
    // 6. Fallback if insufficient stock
    if (qtyToSettle > 0) {
  ...
  Future<void> createPosSale({
    ...
    return _db.transaction(() async {
      ...
      final inventoryAccountId = await _accountsRepo.getAccountIdByName(
        kInventoryAccountName,
      );
      final cogsAccountId = await _accountsRepo.getAccountIdByName(
        kCogsAccountName,
      );
      ...
      for (final item in items) {
        final int costForThisItem = await _calculateCOGSAndConsumeLayers(
          item.product.id,
          item.quantity,
        );
        ...
        final product = await (_db.select(
          _db.products,
        )..where((p) => p.id.equals(item.product.id))).getSingle();
  ...
  Future<void> createPurchaseTransaction({
    ...
    return _db.transaction(() async {
      ...
      for (final item in items) {
        ...
        final double oldTotalValue =
            product.averageCost * product.quantityOnHand;
        ...
        if (newTotalQty > 0) {
          newAverageCost = ((oldTotalValue + newPurchaseValue) / newTotalQty)
              .round();
        }
  ...
  Future<void> createJournalTransaction({
    ...
    if (balance != 0)
      throw Exception(
        'Transaction is unbalanced! Balance diff: $balance cents',
      );
      ...
      await _db
          ...
          .insert(
            TransactionsCompanion.insert(
              id: d.Value(newTransactionId),
              description:
                  "Period Closing Entry (${closingDate.toString().split(' ')[0]})",
